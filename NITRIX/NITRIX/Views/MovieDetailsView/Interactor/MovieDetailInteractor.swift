...
final class MovieDetailInteractor {
    ...
    private func mapMovieWithFavoriteTag(movieNotTagged: MovieDetail) -> MovieDetail {
        ...
        if localDatamanager.existsMovie(movieID: movieNotTagged.movieID) {
            let movieTagged = MovieDetail(
                movieID: movieNotTagged.movieID,
                image: movieNotTagged.image,
                originalTitle: movieNotTagged.originalTitle,
                rating: movieNotTagged.rating,
                releaseDate: movieNotTagged.releaseDate,
                synopsis: movieNotTagged.synopsis,
                title: movieNotTagged.title, favorite: true)
            return movieTagged
        }
...
extension MovieDetailInteractor: MovieDetailInteractorInputProtocol {
    ...
    func addOrRemoveFavorite(
        state: Bool, movieDetail: MovieDetail,
        success: @escaping (() -> ()),
        failure: @escaping ((CoreDataErrors) -> ())) {
            
            localDatamanager?.addOrRemoveFavorite(
                state: state,
                movieDetail: movieDetail,
